...
namespace APP.Data.Servicios
{
    public class MovimientoServicio
    {
        ...
        public async Task<RespuestaConsumidor<RespuestaAPI<IEnumerable<Movimiento>>>> ConsultarMovimientos()
        {
            ...
            try
            {
                respuesta = await Consumidor.Execute<Movimiento, RespuestaAPI<IEnumerable<Movimiento>>>($"https://localhost:7181/api/MovimientoControlador/movimientoscuenta", MethodHttp.GET, null, _protectedLocalStorage);
            }
            catch (Exception ex)
            {
